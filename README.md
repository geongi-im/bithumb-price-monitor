...
- 당일 고가 갱신 시 텔레그램 알림
- 당일 저가 갱신 시 텔레그램 알림
- **120일 차트 이미지 자동 생성 및 전송** (Noto Sans KR 폰트 사용)
- 5일/20일/60일/120일 기간별 최고가/최저가 정보 제공 (차트에 수평선으로 표시)
- 종목별 독립 테이블 관리 (SQLite)
- 최초 실행 시 365일치 과거 데이터 자동 로딩
- 오류 발생 시 테스트 채널로 자동 알림
...
│   └── fonts/                       # 폰트 파일
│       ├── NotoSansKR-Regular.ttf   # Noto Sans KR 일반체
│       └── NotoSansKR-Bold.ttf      # Noto Sans KR 굵은체
└── logs/
...
텔레그램 알림은 **메시지 + 차트 이미지**로 구성됩니다.

### 📊 차트 기능

- **차트 범위**: 최근 120일 일봉 캔들 (365일 데이터 기반 계산)
- **차트 유형**: 캔들스틱 + 거래량 차트
- **데이터 범위**: 365일 데이터 조회 (이동평균선 계산용), 최근 120일만 차트 표시
- **이동평균선**: MA5(녹색), MA20(빨강), MA60(주황), MA120(보라)
  - 5일/20일/60일/120일 이동평균선을 차트에 오버레이
  - 하단 범례로 각 이동평균선 색상 표시
- **폰트**: Noto Sans KR (한글 지원, 크로스 플랫폼 호환)
- **스타일**: 상승(빨강)/하락(파랑) 캔들, 날짜 형식 `yy-mm-dd`
- **Y축 라벨**: 10,000원 이상은 'K' 단위로 표시 (예: 130,000원 → 130K)
- **이동평균선 색상**:
  - 5일선: 녹색 (#2ca02c)
  - 20일선: 빨강 (#d62728)
  - 60일선: 주황 (#ff7f0e)
...
20일최고가: 135,000,000원
60일최고가: 140,000,000원
120일최고가: 145,000,000원

2025-12-20 14:23:45
```

**+ 120일 차트 이미지 첨부** (5일/20일/60일/120일 최고가 라인 표시)
...
**+ 120일 차트 이미지 첨부** (5일/20일/60일/120일 최저가 라인 표시)
